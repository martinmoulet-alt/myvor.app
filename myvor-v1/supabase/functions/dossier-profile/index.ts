...
type Dossier={
  client:string;title:string;objective:string;context?:string;
...
Deno.serve(async(req)=>{
  ...
  const prompt=[
    ...
    JSON.stringify({sector:"string",activity:"string",strategic_issues:["string"],risks_to_avoid:["string"],opportunities:["string"],client_position:"string",key_actors:["string"],watch_topics:["string"],watch_subtopics:["string"],reference_texts:["string"],key_deadlines:["string"],internal_notes:"string"}),
    "DOSSIER :",JSON.stringify({client:clip(dossier.client,300),title:clip(dossier.title,300),objective:clip(dossier.objective,1500),context:clip(dossier.context,2500),watch_keywords:cleanList(dossier.watch_keywords,30),watch_priority_phrases:cleanList(dossier.watch_priority_phrases,20),watch_excluded_keywords:cleanList(dossier.watch_excluded_keywords,20)}),
    "TEXTES DÉJÀ LIÉS (titres uniquement, ne pas inférer leur contenu intégral) :",JSON.stringify(items.map(item=>({title:clip(item.title,500),nature:clip(item.nature,120),urgency:clip(item.urgency,80),source_url:clip(item.source_url,700)}))),
  ].join("\n");

  const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),40000);
  try{
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:Deno.env.get("OPENAI_MODEL")||"gpt-5-mini",input:prompt,max_output_tokens:1800,text:{format:{type:"json_object"}}}),signal:controller.signal});
