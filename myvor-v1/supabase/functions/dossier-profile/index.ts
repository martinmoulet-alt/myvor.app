...
function clip(value:unknown,max:number){return String(value??"").slice(0,max).trim();}
function extractOutputText(payload:any){if(typeof payload?.output_text==="string")return payload.output_text;const chunks=payload?.output?.flatMap((item:any)=>item?.content||[])||[];return chunks.filter((chunk:any)=>chunk?.type==="output_text"||typeof chunk?.text==="string").map((chunk:any)=>chunk?.text||"").join("");}
...
Deno.serve(async(req)=>{
  ...
  const prompt=[
    ...
    "Chaque élément de key_actors doit désigner UNE seule cible identifiable. N'utilise aucune liste interne séparée par des virgules : écris 'et' si nécessaire.",
    "N'inclus jamais les services internes du client (juridique, conformité, marketing, data, RH, communication) dans key_actors.",
    "Évite les catégories vagues comme 'ministères concernés' lorsqu'une institution plus précise peut être déduite sans invention. Si aucune cible précise n'est établie, retourne moins d'acteurs.",
    ...
    "DOSSIER :",
    JSON.stringify({client:clip(dossier.client,300),title:clip(dossier.title,300),objective:clip(dossier.objective,1500),context:clip(dossier.context,2500),watch_keywords:cleanList(dossier.watch_keywords,30),watch_priority_phrases:cleanList(dossier.watch_priority_phrases,20),watch_excluded_keywords:cleanList(dossier.watch_excluded_keywords,20)}),
    "TEXTES DÉJÀ LIÉS (titres uniquement, ne pas inférer leur contenu intégral) :",
    JSON.stringify(items.map(item=>({title:clip(item.title,500),nature:clip(item.nature,120),urgency:clip(item.urgency,80),source_url:clip(item.source_url,700)}))),
  ].join("\n");

  const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),40000);
  try{
    const model=Deno.env.get("OPENAI_MODEL")||"gpt-5-mini";const supportsReasoning=model.startsWith("gpt-5")||/^o\d/.test(model);
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model,input:prompt,max_output_tokens:5000,...(supportsReasoning?{reasoning:{effort:"minimal"}}:{}),store:false,text:{format:{type:"json_schema",name:"myvor_dossier_profile",description:"Profil stratégique structuré d'un dossier client Myvor.",strict:true,schema:PROFILE_SCHEMA}}}),signal:controller.signal});
    if(!response.ok){const raw=await response.text();let message=raw;try{message=JSON.parse(raw)?.error?.message||raw;}catch{}return json({error:`OpenAI a refusé la requête (${response.status}) : ${String(message).slice(0,320)}`},502);}
    const payload=await response.json();const outputText=extractOutputText(payload);const profile=parseJsonObject(outputText);
    if(!profile){const reason=payload?.incomplete_details?.reason||payload?.status||"sortie vide ou invalide";return json({error:`La réponse OpenAI structurée n’était pas exploitable (${reason}).`,diagnostic:String(outputText||"").slice(0,500)},502);}
    return json({profile:{sector:clip(profile.sector,180),activity:clip(profile.activity,700),strategic_issues:cleanList(profile.strategic_issues),risks_to_avoid:cleanList(profile.risks_to_avoid),opportunities:cleanList(profile.opportunities),client_position:clip(profile.client_position,1200),key_actors:cleanActors(profile.key_actors),watch_topics:cleanList(profile.watch_topics),watch_subtopics:cleanList(profile.watch_subtopics),reference_texts:cleanList(profile.reference_texts),key_deadlines:cleanList(profile.key_deadlines),internal_notes:clip(profile.internal_notes,1600)},engine:"myvor-dossier-profile-ai-v6-clean-actors"});
  }catch(error:any){if(error?.name==="AbortError")return json({error:"La génération a dépassé 40 secondes. Réessaie."},504);return json({error:`Erreur de génération : ${error?.message||"inconnue"}`},500);}finally{clearTimeout(timer);}
});
