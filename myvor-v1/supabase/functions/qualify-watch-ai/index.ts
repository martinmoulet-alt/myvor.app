...
async function ask(apiKey:string,item:PendingItem,dossier:Dossier,sourceText:string):Promise<AiResult>{
  ...
  async function callOnce(maxTokens:number){const ctl=new AbortController(),tm=setTimeout(()=>ctl.abort(),30000);try{const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",signal:ctl.signal,headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:Deno.env.get("OPENAI_QUALIFIER_MODEL")||"gpt-5-mini",store:false,instructions,input,reasoning:{effort:"low"},max_output_tokens:maxTokens,text:{verbosity:"low",format:{type:"json_schema",name:"myvor_watch_link_qualification_v4",strict:true,schema}}})});const raw=await r.text();let p:any={};try{p=raw?JSON.parse(raw):{};}catch{throw new Error(`OpenAI JSON HTTP illisible: ${c(raw,100)}`);}if(!r.ok)throw new Error(`OpenAI ${r.status}: ${c(p?.error?.message||raw,180)}`);if(p?.status==="incomplete")throw new Error(`OpenAI incomplete: ${c(p?.incomplete_details?.reason||"inconnue",100)}`);if(p?.status==="failed")throw new Error(`OpenAI failed: ${c(p?.error?.message||"inconnue",120)}`);return p;}finally{clearTimeout(tm);}}
  let p:any;try{p=await callOnce(2200);}catch(e:any){if(!String(e?.message||e).includes("OpenAI incomplete"))throw e;p=await callOnce(3200);}
