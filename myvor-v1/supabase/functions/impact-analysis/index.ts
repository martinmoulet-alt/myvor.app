...
const PROMPT_VERSION="impact-prompt-v6";
...
function buildPrompt(depth:ImpactDepth,client:string,objectif:string,contexte:string,titre:string,lienOfficiel:string,texte:string){
  const depthRule=depth==="express"
    ...
    :depth==="deep"
      ?"ANALYSE APPROFONDIE — DIAGNOSTIC STRATÉGIQUE. Produis une analyse de second niveau, beaucoup plus détaillée qu’une note Standard, sans répétition ni remplissage. Croise toutes les sources disponibles. La Note d’impact doit expliquer en profondeur ce qui change pour le client ; elle ne doit pas se transformer en War Zone. Pour chaque disposition, précise la règle ou évolution, son mécanisme concret, le processus ou l’activité client touché, l’effet attendu, les dépendances et le degré de certitude. Pour chaque risque, structure la description ainsi : CAUSE → MÉCANISME → CONSÉQUENCE CLIENT → EXPOSITION CONCRÈTE → SIGNAL DÉCLENCHEUR À SURVEILLER → MESURE INTERNE À PRÉPARER. Pour chaque opportunité : FENÊTRE → LEVIER → OPTION STRATÉGIQUE → GAIN ATTENDU → CONDITION DE RÉUSSITE → PRÉREQUIS. Pour chaque échéance, explique ce qui se passe à cette date, ce qui change avant/après et ce que le client doit avoir sécurisé ou préparé en amont. Pour chaque recommandation, le champ action doit préciser ACTION CONCRÈTE → PROCESSUS OU LIVRABLE À PRODUIRE → PRÉREQUIS → HORIZON DE MISE EN ŒUVRE → RÉSULTAT OPÉRATIONNEL ATTENDU. Le champ raison doit préciser POURQUOI MAINTENANT → FAIT OU SOURCE QUI JUSTIFIE L’ACTION → RISQUE ÉVITÉ OU OPPORTUNITÉ CAPTÉE → CRITÈRE DE RÉUSSITE. Le champ priorité doit contenir P1, P2 ou P3 avec un horizon temporel. La synthèse doit exposer la situation, le point de bascule, les conséquences juridiques et opérationnelles, les trois décisions internes prioritaires et les principales incertitudes. Chaque justification de sous-score doit comporter plusieurs phrases et expliquer le lien causal avec l’objectif client. N’indique pas de cible d’influence externe, d’acteur à contacter, de canal de lobbying, de méthode d’approche, de message d’influence ni de plan B : ces éléments sont exclusivement réservés à la War Zone. N’invente jamais une date, une position d’acteur ou une preuve ; si une information manque, place-la dans informations_a_confirmer."
