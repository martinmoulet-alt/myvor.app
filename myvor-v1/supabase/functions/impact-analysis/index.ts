...
function parseJson(raw:unknown){let text=String(raw??"").trim().replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"").trim();if(!text)return null;try{return JSON.parse(text);}catch{}const start=text.indexOf("{");const end=text.lastIndexOf("}");if(start>=0&&end>start){try{return JSON.parse(text.slice(start,end+1));}catch{}}return null;}
...
Deno.serve(async(req)=>{
  ...
  const prompt=[
    ...
    "Si une information utile n’est pas vérifiable dans le corpus, place-la dans informations_a_confirmer au lieu de la présenter comme certaine.",
    "Le score mesure l’impact sur l’objectif précis du client, pas l’importance générale du texte.",
    "Barème impératif sur 100 : juridique 0-20 ; économique/opérationnel 0-20 ; urgence institutionnelle 0-15 ; probabilité d’évolution/adoption 0-15 ; politique/réputation 0-15 ; capacité d’action du client 0-15.",
    "Chaque sous-score doit être justifié par le corpus. Le total doit correspondre à la somme des six critères.",
    "Niveaux : faible, moyen, fort, absolument_urgent. Myvor recalculera le niveau à partir du score.",
    "Réponds uniquement en JSON valide avec cette structure exacte :",
    JSON.stringify({synthese:"string",score:0,niveau:"moyen",justification_score:"string",score_detail:{juridique:0,economique_operationnel:0,urgence:0,probabilite:0,politique_reputation:0,capacite_action:0},score_justifications:{juridique:"string",economique_operationnel:"string",urgence:"string",probabilite:"string",politique_reputation:"string",capacite_action:"string"},dispositions_concernees:[{disposition:"string",impact_client:"string",niveau:"moyen"}],risques:[{titre:"string",description:"string",niveau:"moyen"}],opportunites:[{titre:"string",description:"string"}],echeances:[{date:"string",evenement:"string",importance:"string"}],recommandations:[{action:"string",raison:"string",priorite:"string"}],informations_a_confirmer:["string"]}),
    "CLIENT :",client,
