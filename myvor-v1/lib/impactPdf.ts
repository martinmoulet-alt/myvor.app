...
class Composer{
  ...
  paragraph(value:unknown,size=10,color:Color=TEXT,bold=false,indent=0,after=7){const lines=wrapText(value,size,CONTENT_W-indent);const lh=size*1.38;for(const line of lines){this.ensure(lh+after);this.textAt(line,MARGIN+indent,this.y,size,bold,color);this.y-=lh;}this.y-=after;}
  ...
  bullet(value:string,color:Color=TEXT){const lines=wrapText(value,9.5,CONTENT_W-18);const lh=13;for(let i=0;i<lines.length;i++){this.ensure(lh+5);if(i===0)this.textAt("•",MARGIN+2,this.y,11,true,BLUE);this.textAt(lines[i],MARGIN+17,this.y,9.5,false,color);this.y-=lh;}this.y-=5;}
...
export function downloadImpactPdf(note:ImpactPdfNote,dossier:ImpactPdfDossier,mode:string){
  const bytes=buildImpactPdf(note,dossier,mode);const buffer=bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength) as ArrayBuffer;const blob=new Blob([buffer],{type:"application/pdf"});const url=URL.createObjectURL(blob);const anchor=document.createElement("a");const base=String(note.title||dossier.title||"note-impact").normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9]+/g,"-").replace(/^-|-$/g,"").toLowerCase().slice(0,80)||"note-impact";anchor.href=url;anchor.download=`${base}.pdf`;document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);
}
