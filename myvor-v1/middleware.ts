...
const SOURCE_BRIDGE_SECRET_SHA256="fa4d7e910aae2365f72a13e4cb26108526398c8b9b3ee5f134fe9c368a722acf";

function json(error:string,status:number){return NextResponse.json({error},{status,headers:{"Cache-Control":"no-store"}});}
function safeEqual(a:string,b:string){if(a.length!==b.length)return false;let diff=0;for(let index=0;index<a.length;index++)diff|=a.charCodeAt(index)^b.charCodeAt(index);return diff===0;}
...
async function authorizeInternalSourceCollection(request:NextRequest){const supplied=request.headers.get("x-myvor-cron-secret")||"";if(!supplied||supplied.length>256)return false;const suppliedHash=await sha256(supplied);return safeEqual(SOURCE_BRIDGE_SECRET_SHA256,suppliedHash);}
function bodyTooLarge(request:NextRequest,maxBodyBytes?:number){if(!maxBodyBytes)return false;const raw=request.headers.get("content-length");if(!raw)return false;const size=Number(raw);return Number.isFinite(size)&&size>maxBodyBytes;}

export async function middleware(request:NextRequest){
  if(request.nextUrl.pathname==="/api/veille/sources"){
    if(request.method!=="GET")return json("Méthode non autorisée.",405);
    try{if(!await authorizeInternalSourceCollection(request))return json("Accès interne requis.",401);return NextResponse.next();}
    catch{return json("Impossible de vérifier l’accès interne.",503);}
  }
  ...
  try{
    const userResponse=await fetch(`${supabaseUrl}/auth/v1/user`,{method:"GET",headers:{apikey:anonKey,Authorization:authorization},cache:"no-store"});
    ...
    if(route.feature){
      try{
        const quotaResponse=await fetch(`${supabaseUrl}/rest/v1/rpc/consume_ai_quota`,{method:"POST",headers:{apikey:anonKey,Authorization:authorization,"Content-Type":"application/json"},body:JSON.stringify({p_feature:route.feature}),cache:"no-store"});
      ...
      }catch{return json("Impossible de vérifier le quota de sécurité Myvor.",503);}
    }
    return NextResponse.next();
  }catch{return json("Impossible de vérifier la session Myvor.",503);}
}

export const config={matcher:["/api/radar","/api/radar/fast","/api/impact","/api/builder","/api/veille/assign","/api/veille/sources"]};
