...
function sourceTierLabel(tier:SourceTier){return tier===1?"Source primaire":tier===2?"Autorité / doctrine":tier===3?"Expertise institutionnelle":"Actualité / communication";}
...
export default function VeilleCorporate({items,dossiers,add,sync,syncing,syncMessage,link}:{items:Watch[];dossiers:Dossier[];add:()=>void;sync:()=>void;syncing:boolean;syncMessage:string;link:(watchId:string,dossierId:string|null)=>Promise<void>|void}){
  ...
  return <div className={styles.page}>
    <style jsx global>{`.myvor-watch-focus{outline:3px solid #f3bd3e;outline-offset:3px;box-shadow:0 0 0 8px rgba(243,189,62,.12)!important}.myvor-source-legend{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0 14px}.myvor-source-tier{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:5px 9px;font-size:11px;font-weight:850;line-height:1;border:1px solid transparent;white-space:nowrap}.myvor-source-tier::before{content:"";width:7px;height:7px;border-radius:50%;background:currentColor}.myvor-source-tier-1{background:rgba(255,198,42,.14);border-color:rgba(255,198,42,.42);color:#ffc62a}.myvor-source-tier-2{background:rgba(74,163,255,.13);border-color:rgba(74,163,255,.4);color:#69b5ff}.myvor-source-tier-3{background:rgba(168,139,250,.13);border-color:rgba(168,139,250,.38);color:#b9a3ff}.myvor-source-tier-4{background:rgba(132,148,170,.12);border-color:rgba(132,148,170,.3);color:#9bacc2}.myvor-source-footer{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end}@media(max-width:620px){.myvor-source-legend{gap:6px}.myvor-source-tier{font-size:10px;padding:5px 8px}.myvor-source-footer{justify-content:flex-start}}`}</style>
    ...
    <div className="myvor-source-legend" aria-label="Priorité des sources"><span className="myvor-source-tier myvor-source-tier-1">Source primaire</span><span className="myvor-source-tier myvor-source-tier-2">Autorité / doctrine</span><span className="myvor-source-tier myvor-source-tier-3">Expertise institutionnelle</span><span className="myvor-source-tier myvor-source-tier-4">Actualité / communication</span></div>
    ...
    {filtered.length?<div className={styles.grid}>{filtered.map(item=>{const dossier=dossiers.find(d=>d.id===item.dossier_id);const suggestion=visibleSuggestions.find(s=>s.watch_id===item.id);const suggestedDossier=suggestion?dossiers.find(d=>d.id===suggestion.dossier_id):null;const tier=sourceTier(item);const sourceName=item.source_name||sourceLabel(item.source_url);return <article id={`watch-${item.id}`} className={`${styles.card} ${focusId===item.id?"myvor-watch-focus":""}`} key={item.id}><div className={styles.top}><span className={styles.nature}>{item.nature}</span><span className={`${styles.urgency} ${styles[item.urgency.replaceAll(" ","-") as keyof typeof styles]||""}`}>{item.urgency}</span></div><h3 className={styles.title}>{item.title}</h3><div className={styles.meta}><span><CalendarDays size={14}/>{publicationDate(item)}</span><span><Building2 size={14}/>{dossier?`${dossier.client} — ${dossier.title}`:"Aucun dossier"}</span></div>{suggestion&&suggestedDossier&&<div className={styles.suggestion}><div className={styles.suggestionTop}><b>Dossier suggéré : {suggestedDossier.client} — {suggestedDossier.title}</b><span className={styles.confidence}>{Math.round(suggestion.confidence*100)} %</span></div><p>{suggestion.reason}</p><div className={styles.suggestionActions}><button className={styles.accept} onClick={()=>void acceptSuggestion(suggestion)}>Rattacher</button><button className={styles.ignore} onClick={()=>void ignoreSuggestion(item.id)}>Ignorer</button></div></div>}<div className={styles.dossier}><label>Dossier lié</label><select value={item.dossier_id||""} onChange={e=>void manualLink(item.id,e.target.value||null)}><option value="">Non rattaché</option>{dossiers.map(d=><option value={d.id} key={d.id}>{d.client} — {d.title}</option>)}</select></div><div className={styles.footer}>{item.source_url?<a className={styles.source} href={item.source_url} target="_blank" rel="noreferrer">Lire le texte original</a>:<span/>}<span className="myvor-source-footer"><span className={`myvor-source-tier myvor-source-tier-${tier}`} title={`Priorité ${tier} — ${sourceTierLabel(tier)}`}>P{tier} · {sourceTierLabel(tier)}</span><span className={styles.count}>{sourceName}</span></span></div></article>;})}</div>:<div className={styles.empty}><FileText size={34}/><h2>Aucun texte trouvé</h2><p>Synchronisez les sources ou modifiez vos filtres.</p></div>}
  </div>;
}