...
function createdTime(item:Watch){const value=Date.parse(item.created_at);return Number.isFinite(value)?value:0;}
function todayStart(){const now=new Date();return new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime();}
function matchesDateRange(item:Watch,range:string){
  if(range==="all")return true;
  if(range==="today")return createdTime(item)>=todayStart();
  if(range==="published_today"){const published=item.published_at?Date.parse(item.published_at):NaN;return Number.isFinite(published)&&published>=todayStart();}
  const days=range==="7d"?7:range==="30d"?30:range==="90d"?90:0;
  return !days||publicationTime(item)>=Date.now()-days*24*60*60*1000;
}
...
export default function VeilleCorporate({items,dossiers,add,refresh,refreshing,refreshMessage,link}:{items:Watch[];dossiers:Dossier[];add:()=>void;refresh:()=>void;refreshing:boolean;refreshMessage:string;link:(watchId:string,dossierId:string|null)=>Promise<void>|void}){
  ...
  const filtered=useMemo(()=>items.filter(item=>{const q=[item.title,item.nature,item.source_name||""].join(" ").toLowerCase().includes(query.toLowerCase());return q&&matchesDateRange(item,dateRange)&&(nature==="all"||item.nature===nature)&&(urgency==="all"||item.urgency===urgency);}).sort((a,b)=>dateRange==="today"?createdTime(b)-createdTime(a):publicationTime(b)-publicationTime(a)),[items,query,nature,urgency,dateRange]);
