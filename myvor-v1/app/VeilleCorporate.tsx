...
export default function VeilleCorporate({items,dossiers,add,refresh,refreshing,refreshMessage,link}:{items:Watch[];dossiers:Dossier[];add:()=>void;refresh:()=>void;refreshing:boolean;refreshMessage:string;link:(watchId:string,dossierId:string|null)=>Promise<void>|void}){
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.kpis}><div className={styles.kpi}><span>Total surveillé</span><strong>{items.length}</strong><small><FileText size={15}/> Publications suivies</small></div><div className={styles.kpi}><span>Rattachés</span><strong>{linked}</strong><small><Building2 size={15}/> Liés à un dossier</small></div><div className={styles.kpi}><span>À qualifier</span><strong>{unlinked}</strong><small><Search size={15}/> En attente du pipeline</small></div><div className={styles.kpi}><span>Priorités fortes</span><strong>{urgent}</strong><small><AlertTriangle size={15}/> Action rapide</small></div></div>

    <section className={styles.qualification}>
      <div className={styles.qualificationHead}><div><h2><Sparkles size={17}/> Pipeline serveur</h2><p>Myvor collecte, lit et qualifie la veille côté serveur toutes les 15 minutes. Ouvrir cette page ne déclenche plus aucun traitement automatique dans votre navigateur.</p></div><button className={styles.qualificationButton} onClick={refresh} disabled={refreshing}><RefreshCw size={15}/> {refreshing?"Actualisation…":"Actualiser les résultats"}</button></div>
      <div className={styles.qualificationStats}><span>Collecte serveur</span><span>Rattachement automatique à 95 %</span><span>Validation manuelle dès 60 %</span></div>
      <VeilleStatusMessage summary={operationMessage||refreshMessage} technical={operationTechnical}/>
    </section>

    <div className={styles.sourceNotice}><b>Collecte automatique :</b> le cron Supabase exécute le pipeline toutes les 15 minutes, même lorsque personne n’a Myvor ouvert.</div>
