...
function provisionHref(sourceUrl:string,disposition?:Disposition|null){
  ...
  try{
    ...
    const target=sourceTarget(disposition);if(!target)return raw;
    const oldHash=u.hash.replace(/^#/,"").split(":~:")[0];
    const base=`${u.origin}${u.pathname}${u.search}`;
    return `${base}#${oldHash?`${oldHash}:`:""}~:text=${encodeURIComponent(target)}`;
  }catch{return raw;}
}
...
export default function ChangesModuleV4({dossiers,watch}:{dossiers:Dossier[];watch:Watch[];onOpenImpact?:(dossierId:string,watchIds:string[])=>void}){
  ...
  return <div className="page"><style jsx>{`
    ...
    @media(max-width:1050px){.scheme{grid-template-columns:1fr}.arrow{height:18px;transform:rotate(90deg)}}@media(max-width:700px){.page{padding:15px}.head{display:grid}.filters{grid-template-columns:1fr}.changeHead{display:grid}.focusBox{grid-template-columns:auto 1fr}.focusBtn,.sourceBtn{grid-column:1/-1;justify-content:flex-start}}
  `}</style>

  <header className="head"><div><div className="kicker">Comparaison normative du dossier</div><h1>Ce qui change</h1><p>Choisissez un dossier, puis naviguez changement par changement.</p></div><button className={`refresh ${refreshing?"refreshing":""}`} onClick={refresh}><RefreshCw size={14}/>Actualiser</button></header>
