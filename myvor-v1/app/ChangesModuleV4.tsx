...
function historicalMessage(item:Watch,path:Watch[]){
  if(item.historical_status==="resolved")return{label:"Historique normatif établi",tone:"ok",detail:`${path.length} état(s) antérieur(s) démontré(s). Le dernier constitue le prédécesseur direct.`};
  if(item.historical_status==="partial")return{label:"Historique normatif partiel",tone:"warn",detail:`${path.length} état(s) antérieur(s) démontré(s), mais la chaîne reste à consolider.`};
  if(item.historical_status==="unresolved")return{label:"Prédécesseur non résolu",tone:"warn",detail:"Le texte annonce une mutation d’une norme antérieure, mais Myvor n’a pas encore résolu avec certitude le texte précédent."};
  if(item.historical_status==="no_history")return{label:"Aucun état antérieur démontré",tone:"neutral",detail:"Aucune succession normative antérieure n’est établie pour ce texte ; il n’est pas assimilé artificiellement à une modification."};
  return{label:"Historique en cours",tone:"neutral",detail:"Myvor vérifie les états successifs de la norme avant d’établir le comparatif."};
}
...
export default function ChangesModuleV4({dossiers,watch}:{dossiers:Dossier[];watch:Watch[];onOpenImpact?:(dossierId:string,watchIds:string[])=>void}){
  ...
  return <div className="page"><style jsx>{`
    ...
    .historyBand{display:grid;gap:10px;padding:12px 2px 3px;border-bottom:1px solid #173653}.historyHead{display:flex;align-items:flex-start;justify-content:space-between;gap:15px}.historyHead>div{display:grid;gap:3px}.historyHead span{font-size:8.5px;text-transform:uppercase;letter-spacing:.08em;font-weight:900;color:#f3bd3e}.historyHead p{margin:0;color:#8196ac;font-size:9.5px}.historyStatus{font-size:8px!important;letter-spacing:.04em!important;text-transform:none!important;color:#9eb0c4!important}.historyStatus.ok{color:#7fe0b4!important}.historyStatus.warn{color:#f3bd3e!important}.historyFlow{display:flex;align-items:flex-start;min-width:0;overflow-x:auto;padding:4px 0 10px}.historyStep{width:190px;min-width:190px;display:grid;grid-template-columns:10px 1fr;column-gap:8px;position:relative}.historyStep:after{content:"";position:absolute;top:5px;left:9px;width:calc(100% - 1px);height:1px;background:#29445f}.historyStep:last-child:after{display:none}.historyDot{width:9px;height:9px;border-radius:50%;border:2px solid #71869e;background:#071c33;position:relative;z-index:2}.historyStep.current .historyDot{border-color:#f3bd3e;background:#f3bd3e;box-shadow:0 0 0 4px rgba(243,189,62,.08)}.historyText{display:grid;gap:3px;padding-right:16px}.historyText small{font-size:7.5px;color:#71869e;text-transform:uppercase;letter-spacing:.04em}.historyText strong{font-size:9px;line-height:1.32;color:#dfe8f1}.historyStep.current .historyText strong{color:#fff}.historyEmpty{border:1px dashed #29445f;border-radius:8px;padding:10px 11px;color:#8196ac;font-size:9.5px;line-height:1.45}.historyEmpty.warn{border-color:#6f5721;color:#d6bf83}
    ...
    @media(max-width:700px){.page{margin:-20px -18px -28px;padding:20px 18px 28px}.head{flex-direction:column}.refresh{width:100%;justify-content:center}.tabs{gap:16px;overflow-x:auto}.tabs button{white-space:nowrap}.contextBar{display:grid}.contextControls{display:grid}.contextBar label{display:grid;gap:5px}.contextBar select{min-width:0;width:100%}.contextMeta{flex-wrap:wrap}.changeHead{display:grid}.historyHead{display:grid}.focusBox{grid-template-columns:auto 1fr}.focusBtn,.sourceBtn{grid-column:1/-1;justify-content:flex-start}.sourceRow{grid-template-columns:1fr}}
  `}</style>
  ...
  <section className="contextBar"><div className="contextControls"><label><span>Dossier</span><select value={selectedDossierId} onChange={e=>{setSelectedDossierId(e.target.value);setFilter("all");setSelectedChangeId("");}}>{dossiers.map(d=><option key={d.id} value={d.id}>{d.client} — {d.title}</option>)}</select></label><label><span>Qualification</span><select value={filter} onChange={e=>{setFilter(e.target.value as Filter);setSelectedChangeId("");}}>{FILTERS.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label></div><div className="contextMeta"><span><FileText size={14}/>{filtered.length} évolution{filtered.length>1?"s":""}</span><span><ShieldCheck size={14}/>{view==="evolution"?(item?.historical_status==="resolved"?"Historique établi":item?.historical_status==="partial"?"Historique partiel":item?.historical_status==="unresolved"?"Prédécesseur à résoudre":"Analyse historique"):item?.normative_chain_status==="complete"?"Chaîne vérifiée":item?.normative_chain_status==="partial"?"À consolider":"Analyse en cours"}</span></div></section>
  ...
  {view==="evolution"&&<>{historicalState&&<div className="historyBand"><div className="historyHead"><div><span>Historique de la norme</span><p>{historicalState.detail}</p></div><span className={`historyStatus ${historicalState.tone}`}>{historicalState.label}</span></div>{historicalPath.length?<div className="historyFlow">{historicalPath.map(w=><div key={w.id} className="historyStep"><i className="historyDot"/><div className="historyText"><small>{dateLabel(w.published_at||w.created_at)} · {normKind(w)}</small><strong title={w.title}>{cut(w.title,95)}</strong></div></div>)}<div className="historyStep current"><i className="historyDot"/><div className="historyText"><small>{dateLabel(item.published_at||item.created_at)} · État nouveau</small><strong title={item.title}>{cut(item.title,95)}</strong></div></div></div>:<div className={`historyEmpty ${item.historical_status==="unresolved"?"warn":""}`}>{historicalState.detail}</div>}</div>}
