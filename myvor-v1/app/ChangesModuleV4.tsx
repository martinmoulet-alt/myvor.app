...
export default function ChangesModuleV4({dossiers,watch,onOpenImpact}:{dossiers:Dossier[];watch:Watch[];onOpenImpact?:(dossierId:string,watchIds:string[])=>void}){
  ...
  return <div className="page"><style jsx>{`
    ...
    .tabs{display:inline-flex;align-items:center;gap:2px;width:max-content;max-width:100%;padding:3px;border:1px solid #173653;border-radius:8px;background:#061a30;box-shadow:0 4px 14px rgba(0,0,0,.08)}.tabs button{border:0;background:transparent;color:#8296ad;padding:6px 9px;border-radius:6px;font-size:9.5px;line-height:1;font-weight:850;letter-spacing:.01em;cursor:pointer;white-space:nowrap;transition:background .12s,color .12s}.tabs button:hover{color:#dbe7f3;background:#0a243f}.tabs .tabActive{color:#07162c;background:#f3bd3e}.tabs .tabActive:hover{color:#07162c;background:#f3bd3e}
    ...
    @media(max-width:760px){.page{margin:-20px -18px -28px;padding:20px 18px 28px}.head{flex-direction:column}.refresh{width:100%;justify-content:center}.tabs{max-width:100%;overflow-x:auto}.tabs button{flex:0 0 auto}.contextBar{display:grid}.contextControls{display:grid}.contextBar label{display:grid;gap:5px}.contextBar select{min-width:0;width:100%}.contextMeta{flex-wrap:wrap}.overview{grid-template-columns:1fr}.overviewMain{grid-column:auto}.metric{border-top:1px solid #173653}.actionStrip{grid-template-columns:auto 1fr}.actionBtns{grid-column:1/-1}.actRow{grid-template-columns:80px 1fr 24px}.actRole,.actText,.actImpact{display:none}.changeHead{display:grid}.focusBox{grid-template-columns:auto 1fr}.focusBtn,.sourceBtn{grid-column:1/-1;justify-content:flex-start}.sourceRow{grid-template-columns:1fr}.sourceRow em{display:none}}
  `}</style>
  ...
  <div className="tabs" role="tablist" aria-label="Vues du module Ce qui change">
    <button type="button" title="Évolution normative" aria-label="Évolution normative" className={view==="evolution"?"tabActive":""} onClick={()=>setView("evolution")}>Évolution</button>
    <button type="button" title="Filiation normative" aria-label="Filiation normative" className={view==="filiation"?"tabActive":""} onClick={()=>setView("filiation")}>Filiation</button>
    <button type="button" title="Fondements juridiques" aria-label="Fondements juridiques" className={view==="fondements"?"tabActive":""} onClick={()=>setView("fondements")}>Fondements</button>
  </div>
