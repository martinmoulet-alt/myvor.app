...
export default function ImpactModule({dossiers,watch,onActions}:{dossiers:Dossier[];watch:Watch[];onActions?:(drafts:ActionDraft[])=>Promise<void>|void}){
  ...
  function exportPdf(){
    ...
    const sources=note.sources_used?.length?`<section><h2>Sources analysées</h2>${note.sources_used.map(source=>`<div class='source'><span>${escapeHtml(source.title)}</span>${source.url?`<small>${escapeHtml(source.url)}</small>`:""}</div>`).join("")}</section>`:"";
