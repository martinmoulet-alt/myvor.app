...
export default function BuilderModule({dossiers,watch}:{dossiers:Dossier[];watch:Watch[]}){
  ...
  async function generate(){
    ...
    try{
      ...
      const nextDocument=sanitizeDocument(payload.document as BuiltDocument);setDocument(nextDocument);setSelection({start:0,end:0,text:""});
  ...
  function exportPdf(){
    if(!document||!dossier)return;setError("");
  ...
  function exportWord(){
    ...
    const html=`<!doctype html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" lang="fr"><head><meta charset="utf-8"><title>${escapeHtml(document.title)}</title><style>body{font-family:Arial,sans-serif;color:#14213d;line-height:1.55;margin:40px}h1{color:#071936;font-size:24px;border-bottom:2px solid #d0a740;padding-bottom:12px}h2{font-size:15px;color:#071936;margin-top:24px}.meta{color:#66758a;font-size:11px}.subject{background:#f4f6f9;padding:10px;margin:15px 0}.body p{margin:0 0 12px}li{margin-bottom:7px}</style></head><body><div class="meta">MYVOR — Note Builder · ${escapeHtml(dossier.client)} · ${escapeHtml(new Date().toLocaleDateString("fr-FR"))}</div><h1>${escapeHtml(document.title)}</h1>${document.subject?`<div class="subject"><strong>Objet :</strong> ${escapeHtml(document.subject)}</div>`:""}<div class="body">${documentBodyHtml(document.content)}</div>${document.key_points?.length?`<h2>Points clés</h2><ul>${document.key_points.map(point=>`<li>${escapeHtml(point)}</li>`).join("")}</ul>`:""}${sources}</body></html>`;
  ...
  function discardDraft(){if(!dossierId)return;clearBuilderDraft(dossierId);setInstruction("");setDocument(null);setSelection({start:0,end:0,text:""});setDraftMessage("Brouillon supprimé.");setError("");setSaveMessage("");}
