...
const formats=[["note-client","Note client"],["argumentaire","Argumentaire"],["email","E-mail"],["rendez-vous","Préparation de rendez-vous"]] as const;
...
export default function BuilderModule({dossiers,watch}:{dossiers:Dossier[];watch:Watch[]}){
  ...
  useEffect(()=>{
    ...
    const savedAt=new Date(draft.saved_at);setDraftMessage(`Brouillon repris${Number.isFinite(savedAt.getTime())?` · sauvegardé le ${savedAt.toLocaleDateString("fr-FR")} à ${savedAt.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}`:""}.`);
  },[dossierId]);
  ...
  useEffect(()=>{
    ...
    draftTimer.current=window.setTimeout(()=>{
      ...
      setDraftMessage(saved?"Brouillon sauvegardé automatiquement sur cet appareil.":"Sauvegarde locale indisponible sur cet appareil.");
    },700);
  ...
  async function generate(){
    ...
    try{
      ...
      const rawDocument=payload.document as BuiltDocument;const nextDocument=sanitizeDocument(rawDocument);setDocument(nextDocument);
      const saved=await saveProduction({dossier_id:dossier.id,type:"builder",title:nextDocument.title||`Document — ${dossier.title}`,content:{document:nextDocument,raw_document:rawDocument,format,audience,tone,instruction,item_ids:related.map(i=>i.id),context_used:payload.context_used||null,engine:payload.engine||"supabase-note-builder"}});
    ...
    }catch(err:any){const message=String(err?.message||"");setError(message.includes("Failed to send a request")?"Impossible de joindre la fonction Supabase note-builder. Le brouillon actuel reste sauvegardé.":message||"Génération impossible");}finally{setLoading(false);}
  }

  async function copyDocument(){if(!document)return;const text=[document.title,document.subject?`Objet : ${document.subject}`:"",document.content,document.key_points?.length?`\nPoints clés\n${document.key_points.map(x=>`• ${x}`).join("\n")}`:""].filter(Boolean).join("\n\n");try{await navigator.clipboard.writeText(text);setCopied(true);setTimeout(()=>setCopied(false),1800);}catch{setError("La copie a été bloquée par le navigateur. Sélectionne le texte manuellement.");}}
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.setup}><section className={styles.panel}><h2>1. Choisir la base</h2><div className={styles.field}><label>Dossier client</label><select value={dossierId} onChange={e=>{setDossierId(e.target.value);hydratedDraft.current="";setDocument(null);setError("");setSaveMessage("");setDraftMessage("");}}><option value="">Sélectionner un dossier</option>{dossiers.map(d=><option key={d.id} value={d.id}>{d.client} — {d.title}</option>)}</select></div>{dossier&&<div className={styles.summary}><b>Objectif client :</b><br/>{dossier.objective}<br/><br/><b>Textes liés :</b> {related.length}</div>}</section>
      <section className={styles.panel}><h2>2. Paramétrer le document</h2><div className={styles.formats}>{formats.map(([id,label])=><button type="button" key={id} className={`${styles.formatButton} ${format===id?styles.active:""}`} onClick={()=>setFormat(id)}>{label}</button>)}</div><div className={styles.field}><label>Public visé</label><input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Client, député, cabinet ministériel…"/></div><div className={styles.field}><label>Ton</label><select value={tone} onChange={e=>setTone(e.target.value)}><option>professionnel et direct</option><option>institutionnel et diplomatique</option><option>convaincant et offensif</option><option>pédagogique et synthétique</option></select></div><div className={styles.field}><label>Instruction complémentaire</label><textarea value={instruction} onChange={e=>setInstruction(e.target.value)} placeholder="Ex. Insister sur le coût économique et proposer un rendez-vous avant l’examen en commission."/></div></section></div>
    <div className={styles.generate}><div><h3>Prêt à rédiger</h3><p>{related.length} texte(s) lié(s) au dossier sélectionné.</p>{draftMessage&&<small style={{display:"block",marginTop:6,opacity:.72}}>{draftMessage}</small>}</div><div style={{display:"flex",gap:10,flexWrap:"wrap",justifyContent:"flex-end"}}>{dossierId&&<button type="button" onClick={discardDraft} title="Supprimer le brouillon local" style={{background:"transparent",border:"1px solid rgba(255,255,255,.14)"}}><Trash2 size={16}/>Effacer le brouillon</button>}<button onClick={generate} disabled={loading||!dossier||!related.length}><Sparkles size={17}/>{loading?"Rédaction en cours…":"Générer le document"}</button></div></div>
    {error&&<div className={styles.error}>{error}</div>}{saveMessage&&<div className={styles.error}>{saveMessage}</div>}
    {document&&<section className={styles.document}><div className={styles.documentHead}><div><div className={styles.eyebrow}>Document généré</div><h2>{document.title}</h2>{document.subject&&<div className={styles.subject}><b>Objet :</b> {document.subject}</div>}</div><div className={styles.documentActions}><button className={styles.pdf} onClick={exportPdf}><Download size={16}/>Exporter PDF</button><button className={styles.copy} onClick={copyDocument}><Copy size={16}/>{copied?"Copié":"Copier"}</button></div></div>{paragraphs.length>0&&<div className={styles.content}>{paragraphs.map((paragraph,index)=><p key={index} className={styles.contentParagraph}>{paragraph}</p>)}</div>}{!!document.key_points?.length&&<div className={styles.points}><h3>Points clés</h3><ul>{document.key_points.map((point,index)=><li key={index}>{point}</li>)}</ul></div>}{!!document.sources?.length&&<div className={styles.sources}><h3>Sources utilisées</h3>{document.sources.map((source,index)=><div className={styles.sourceRow} key={`${source.url}-${index}`}><span><FileText size={15}/>{source.title}</span>{source.url&&<a href={source.url} target="_blank" rel="noreferrer">Lire la source</a>}</div>)}</div>}</section>}
  </div>;
}