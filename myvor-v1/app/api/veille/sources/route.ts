...
function parseFeed(xml:string,source:Source):FeedItem[]{const rss=[...xml.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item>/gi)].map(m=>m[1]);const atom=[...xml.matchAll(/<entry\b[^>]*>([\s\S]*?)<\/entry>/gi)].map(m=>m[1]);const blocks=rss.length?rss:atom;return blocks.map(block=>{const title=first(block,["title"]);const excerpt=first(block,["description","summary","content:encoded","content","subtitle"]);const source_url=linkFrom(block);return{title,nature:inferNature(title,source.defaultNature,`${source.name} ${source_url}`),source_url,source_name:source.name,published_at:first(block,["pubDate","published","updated","dc:date"]),excerpt:excerpt.slice(0,2200)||undefined};}).filter(x=>x.title&&x.source_url.startsWith("http")&&!isGenericNavigationTitle(x.title));}
async function fetchFeed(source:Source){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),6500);try{const response=await fetch(source.url,{headers:{"User-Agent":"Mozilla/5.0 Myvor/1.0","Accept":"application/rss+xml,application/atom+xml,application/xml,text/xml,*/*"},next:{revalidate:300},signal:controller.signal});if(!response.ok)throw new Error(`${source.name}: HTTP ${response.status}`);return parseFeed(await responseText(response),source).slice(0,12);}finally{clearTimeout(timer);}}

async function fetchHtmlListing(input:{name:string;url:string;base:string;pathPattern:RegExp;defaultNature:string;limit?:number}):Promise<FeedItem[]>{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),6500);try{const response=await fetch(input.url,{headers:{"User-Agent":"Mozilla/5.0 Myvor/1.0","Accept":"text/html,*/*","Accept-Language":"fr-FR,fr;q=0.9"},next:{revalidate:600},signal:controller.signal});if(!response.ok)throw new Error(`${input.name}: HTTP ${response.status}`);const html=await responseText(response);const items:FeedItem[]=[];const regex=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;for(const match of html.matchAll(regex)){const href=decodeHtml(match[1]);if(!input.pathPattern.test(href))continue;const title=decodeHtml(match[2]);if(title.length<12||isGenericNavigationTitle(title))continue;const source_url=href.startsWith("http")?href:`${input.base}${href.startsWith("/")?"":"/"}${href}`;const excerpt=excerptAround(html,match.index||0,title);if(excerpt.length<60)continue;items.push({title,nature:inferNature(title,input.defaultNature,`${input.name} ${source_url}`),source_url,source_name:input.name,excerpt:excerpt||undefined});}return items.filter((x,i,a)=>a.findIndex(y=>y.source_url===x.source_url)===i).slice(0,input.limit||12);}finally{clearTimeout(timer);}}

async function fetchLegifranceJorf():Promise<FeedItem[]>{const response=await fetch("https://www.legifrance.gouv.fr/jorf/jo",{redirect:"follow",headers:{"User-Agent":"Mozilla/5.0 Myvor/1.0","Accept":"text/html,application/xhtml+xml,*/*","Accept-Language":"fr-FR,fr;q=0.9"},next:{revalidate:300}});if(!response.ok)throw new Error(`Légifrance — JORF: HTTP ${response.status}`);const html=await responseText(response);const issueDate=decodeHtml(html.match(/Journal officiel de la République française[^<]*du\s+([^<]+)/i)?.[1]||"");const items:FeedItem[]=[];const regex=/<a\b[^>]*href=["']([^"']*\/jorf\/id\/JORFTEXT[^"']*)["'][^>]*>([\s\S]*?)<\/a>/gi;for(const match of html.matchAll(regex)){const title=decodeHtml(match[2]);if(!title||isGenericNavigationTitle(title))continue;const nature=inferNature(title,"Texte réglementaire");if(!["Loi","Ordonnance","Décret","Arrêté","Décision / jurisprudence","Rapport"].includes(nature))continue;const href=match[1];const source_url=href.startsWith("http")?href:`https://www.legifrance.gouv.fr${href.startsWith("/")?"":"/"}${href}`;const excerpt=excerptAround(html,match.index||0,title);items.push({title,nature,source_url,source_name:"Légifrance — Journal officiel",published_at:issueDate,excerpt:excerpt||undefined});}return items.filter((x,i,a)=>a.findIndex(y=>y.source_url===x.source_url)===i).slice(0,20);}
async function fetchViePubliqueReports():Promise<FeedItem[]>{return fetchHtmlListing({name:"Vie-publique — Rapports",url:"https://www.vie-publique.fr/bibliotheque-rapports-publics",base:"https://www.vie-publique.fr",pathPattern:/\/rapport\//i,defaultNature:"Rapport",limit:15});}
async function fetchConseilConstitutionnel():Promise<FeedItem[]>{return fetchHtmlListing({name:"Conseil constitutionnel",url:"https://qpc360.conseil-constitutionnel.fr/",base:"https://qpc360.conseil-constitutionnel.fr",pathPattern:/(decision|decisions|qpc)/i,defaultNature:"Décision / jurisprudence",limit:12});}
async function fetchCnil():Promise<FeedItem[]>{return fetchHtmlListing({name:"CNIL",url:"https://www.cnil.fr/fr/actualite",base:"https://www.cnil.fr",pathPattern:/\/fr\/(?!actualite\/?$)(?:[a-z0-9-]+)(?:\/|$)/i,defaultNature:"Communiqué institutionnel",limit:15});}
async function fetchArcep():Promise<FeedItem[]>{return fetchHtmlListing({name:"ARCEP",url:"https://www.arcep.fr/actualites.html",base:"https://www.arcep.fr",pathPattern:/(communiques-de-presse|actualites\/actualites-et-communiques|uploads\/tx_gspublication|consultations-publiques)/i,defaultNature:"Communiqué institutionnel",limit:15});}
async function fetchEurLex():Promise<FeedItem[]>{const response=await fetch("https://eur-lex.europa.eu/oj/direct-access.html?locale=fr",{headers:{"User-Agent":"Mozilla/5.0 Myvor/1.0","Accept":"text/html,*/*","Accept-Language":"fr-FR,fr;q=0.9"},next:{revalidate:600}});if(!response.ok)throw new Error(`EUR-Lex: HTTP ${response.status}`);const html=await responseText(response);const items:FeedItem[]=[];const regex=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;for(const match of html.matchAll(regex)){const href=decodeHtml(match[1]);const title=decodeHtml(match[2]);if(title.length<8||isGenericNavigationTitle(title))continue;if(!/(legal-content|oj\/daily-view|oj\/direct-access)/i.test(href))continue;if(!/(règlement|reglement|directive|décision|decision|journal officiel|législation|legislation)/i.test(title))continue;const source_url=href.startsWith("http")?href:`https://eur-lex.europa.eu${href.startsWith("/")?"":"/"}${href}`;const excerpt=excerptAround(html,match.index||0,title);items.push({title,nature:inferNature(title,"Acte de l’Union européenne",source_url),source_url,source_name:"EUR-Lex",excerpt:excerpt||undefined});}return items.filter((x,i,a)=>a.findIndex(y=>y.source_url===x.source_url)===i).slice(0,18);}

export async function GET(){const feedSettled=await Promise.allSettled(SOURCES.map(fetchFeed));const extraNames=["Légifrance — Journal officiel","Vie-publique — Rapports","Conseil constitutionnel","CNIL","ARCEP","EUR-Lex"];const extras=await Promise.allSettled([fetchLegifranceJorf(),fetchViePubliqueReports(),fetchConseilConstitutionnel(),fetchCnil(),fetchArcep(),fetchEurLex()]);const items=[...extras.flatMap(r=>r.status==="fulfilled"?r.value:[]),...feedSettled.flatMap(r=>r.status==="fulfilled"?r.value:[])].filter(item=>!isGenericNavigationTitle(item.title)).filter((x,i,a)=>a.findIndex(y=>y.source_url===x.source_url)===i).slice(0,160);const active_sources=[...feedSettled.map((r,i)=>r.status==="fulfilled"?SOURCES[i].name:null).filter(Boolean),...extras.map((r,i)=>r.status==="fulfilled"?extraNames[i]:null).filter(Boolean)];const unavailable_sources=[...feedSettled.map((r,i)=>r.status==="rejected"?SOURCES[i].name:null).filter(Boolean),...extras.map((r,i)=>r.status==="rejected"?extraNames[i]:null).filter(Boolean)];const unavailable_details=[...feedSettled.map((r,i)=>r.status==="rejected"?`${SOURCES[i].name}: ${String(r.reason?.message||r.reason)}`:null).filter(Boolean),...extras.map((r,i)=>r.status==="rejected"?`${extraNames[i]}: ${String((r as PromiseRejectedResult).reason?.message||(r as PromiseRejectedResult).reason)}`:null).filter(Boolean)];return NextResponse.json({synced_at:new Date().toISOString(),active_sources,unavailable_sources,unavailable_details,items});}
