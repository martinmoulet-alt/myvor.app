...
const WEAK_ANCHORS=new Set([
  "article","commerci","profession","entrepri","consomma","service","information","droit","regle","modal","public","activit",
  "aout","juillet","juin","mai","avril","mars","fevrier","janvier","septembr","octobr","novembr","decembr",
  "decret","arrete","ordonnance","code","applic","portant","journal","officiel","france","francais","entree","vigueur",
  "publication","communiqu","actualit","autorite","minister","gouvernement","parlement"
]);
...
function strongKeywords(value:string){return keywords(value).filter(word=>!WEAK_ANCHORS.has(word));}
...
function phraseIsDiscriminating(phrase:string){const normalized=normalize(phrase);if(!normalized)return false;const tokens=normalized.split(/\s+/).filter(Boolean);const strong=strongKeywords(phrase);return strong.length>=1&&(tokens.length>=2||strong.length>=2||/\d/.test(normalized));}
...
async function fetchOfficialText(url:string){if(!url||!isOfficialUrl(url))return{full:"",lead:""};const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),7000);try{const response=await fetch(url,{redirect:"follow",headers:{"User-Agent":"Mozilla/5.0 Myvor/7.0","Accept":"text/html,application/xhtml+xml,*/*","Accept-Language":"fr-FR,fr;q=0.9"},signal:controller.signal,cache:"no-store"});if(!response.ok||!/text\/html|application\/xhtml\+xml/i.test(response.headers.get("content-type")||""))return{full:"",lead:""};const html=await response.text();const meta=metaDescription(html);const body=readableBody(html);return{full:[meta,body].filter(Boolean).join("\n").slice(0,MAX_READABLE_TEXT),lead:[meta,body.slice(0,2200)].filter(Boolean).join(" ").slice(0,3000)};}catch{return{full:"",lead:""};}finally{clearTimeout(timer);}}
async function enrichItems(items:WatchItem[]){return Promise.all(items.map(async item=>{const existing=String(item.excerpt||"").trim();const fetched=await fetchOfficialText(String(item.source_url||""));return{...item,excerpt:[fetched.full,existing].filter(Boolean).join("\n").slice(0,MAX_READABLE_TEXT),_fullTextChars:fetched.full.length,_lead:fetched.lead||existing.slice(0,2200)};}));}
...
function keywordScore(item:WatchItem,dossier:Dossier):ScoreResult{
  ...
  const fullWords=new Set(strongKeywords(normalizedFull));
  const leadWords=new Set(strongKeywords(normalizedLead));
  const titleWords=new Set(strongKeywords(normalizedTitle));
  ...
  const discriminatingPriority=priorityPhrases.filter(phraseIsDiscriminating);
  const discriminatingExplicit=explicitKeywords.filter(phraseIsDiscriminating);
  const priorityMatches=discriminatingPriority.filter(phrase=>containsPhrase(normalizedLead,phrase));
  const phraseMatches=discriminatingExplicit.filter(phrase=>containsPhrase(normalizedLead,phrase));
  const bodyPriorityMatches=discriminatingPriority.filter(phrase=>containsPhrase(normalizedFull,phrase));
  const bodyPhraseMatches=discriminatingExplicit.filter(phrase=>containsPhrase(normalizedFull,phrase));

  const anchors=[...new Set(strongKeywords(`${dossier.title} ${priorityPhrases.join(" ")}`))];
  ...
  const explicitWords=[...new Set(explicitKeywords.flatMap(value=>strongKeywords(value)))];
  const titleExplicitMatches=explicitWords.filter(word=>matchKeyword(titleWords,word));
  const leadExplicitMatches=explicitWords.filter(word=>matchKeyword(leadWords,word));
  const fullExplicitMatches=explicitWords.filter(word=>matchKeyword(fullWords,word));
  const matches=[...new Set([...titleExplicitMatches,...leadExplicitMatches,...fullExplicitMatches])];

  const priorityInTitle=priorityMatches.filter(phrase=>containsPhrase(normalizedTitle,phrase));
  const phraseInTitle=phraseMatches.filter(phrase=>containsPhrase(normalizedTitle,phrase));
  let score=0;

  if(priorityInTitle.length>=1)score=.99;
  else if(priorityMatches.length>=1)score=.97;
  else if(phraseInTitle.length>=2)score=.99;
  else if(phraseInTitle.length===1)score=.97;
  else if(phraseMatches.length>=2)score=.96;
  else if(phraseMatches.length===1)score=.93;
  else if(titleAnchors.length>=2&&leadAnchors.length>=2)score=.95;
  else if(titleAnchors.length>=1&&leadAnchors.length>=3)score=.92;
  else if(titleExplicitMatches.length>=2&&leadExplicitMatches.length>=3)score=.94;
  else if(titleExplicitMatches.length>=1&&leadExplicitMatches.length>=3)score=.91;
  else if(bodyPriorityMatches.length>=1)score=.84;
  else if(bodyPhraseMatches.length>=1)score=.80;
  else if(titleExplicitMatches.length>=1&&fullExplicitMatches.length>=2)score=.78;
  else if(fullExplicitMatches.length>=3)score=.72;
  else score=0;
...
function keywordAssignments(items:WatchItem[],dossiers:Dossier[]):Assignment[]{return items.map(item=>{
  ...
  if(!best||best.score<MIN_ACTIONABLE_CONFIDENCE){const blocked=ranked.find(result=>result.blockedBy);return{watch_id:item.id,dossier_id:null,confidence:Number((best?.score||0).toFixed(2)),reason:blocked?`Exclusion détectée : ${blocked.blockedBy}.`:`Pertinence thématique insuffisante (< ${Math.round(MIN_ACTIONABLE_CONFIDENCE*100)} %).`};}
  ...
  const evidence=[best.priorityMatches.length?`Expression prioritaire : ${best.priorityMatches.join(", ")}.`:"",best.phraseMatches.length?`Expression de veille : ${best.phraseMatches.join(", ")}.`:"",best.anchors.length?`Ancrages thématiques : ${best.anchors.join(", ")}.`:"",best.matches.length?`Mots-clés thématiques : ${best.matches.join(", ")}.`:""].filter(Boolean).join(" ");
  return{watch_id:item.id,dossier_id:best.dossier.id,confidence:Number(confidence.toFixed(2)),reason:`${evidence||`Correspondance thématique forte avec ${best.dossier.title}.`} Signal confirmé dans le titre ou le résumé.`};
});}
...
export async function POST(request:Request){
  ...
  if(!rawItems.length||!dossiers.length)return NextResponse.json({assignments:[],engine:"myvor-relevance-precision-v7",enriched:0,full_text_chars:0,threshold:MIN_ACTIONABLE_CONFIDENCE});
  ...
  return NextResponse.json({assignments,engine:"myvor-relevance-precision-v7",enriched,full_text_chars:fullTextChars,threshold:MIN_ACTIONABLE_CONFIDENCE});
}
