...
async function fetchOfficialText(url:string){if(!url||!isOfficialUrl(url))return "";const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),7000);try{const response=await fetch(url,{redirect:"follow",headers:{"User-Agent":"Mozilla/5.0 Myvor/4.0","Accept":"text/html,application/xhtml+xml,*/*","Accept-Language":"fr-FR,fr;q=0.9"},signal:controller.signal,cache:"no-store"});if(!response.ok)return "";const contentType=response.headers.get("content-type")||"";if(!/text\/html|application\/xhtml\+xml/i.test(contentType))return "";const html=await response.text();const meta=metaDescription(html);const body=readableBody(html);return [meta,body].filter(Boolean).join("\n").replace(/[ \t]+/g," ").replace(/\n\s*\n+/g,"\n").trim().slice(0,MAX_READABLE_TEXT);}catch{return "";}finally{clearTimeout(timer);}}
async function enrichItems(items:WatchItem[]){return Promise.all(items.map(async item=>{const existing=String(item.excerpt||"").trim();const full=await fetchOfficialText(String(item.source_url||""));return{...item,excerpt:[full,existing].filter(Boolean).join("\n").slice(0,MAX_READABLE_TEXT),_fullTextChars:full.length};}));}
...
function keywordAssignments(items:WatchItem[],dossiers:Dossier[]):Assignment[]{return items.map(item=>{const ranked=dossiers.map(dossier=>({dossier,...keywordScore(item,dossier)})).sort((a,b)=>b.score-a.score);const best=ranked[0];const second=ranked[1];if(!best||best.score<0.55){const blocked=ranked.find(result=>result.blockedBy);return{watch_id:item.id,dossier_id:null,confidence:0,reason:blocked?`Exclusion détectée : ${blocked.blockedBy}.`:"Aucun mot-clé suffisamment pertinent détecté dans le texte officiel complet."};}let confidence=best.score;if(second&&second.score>=0.55&&(best.score-second.score)<0.12)confidence=Math.min(confidence,0.88);const details=[best.priorityMatches.length?`Expression prioritaire : ${best.priorityMatches.join(", ")}.`:"",best.matches.length?`${best.explicitKeywords?"Mots-clés explicites":"Mots-clés détectés"} : ${best.matches.join(", ")}.`:""].filter(Boolean).join(" ");return{watch_id:item.id,dossier_id:best.dossier.id,confidence:Number(confidence.toFixed(2)),reason:`${details||`Correspondance avec ${best.dossier.title}.`} Analyse du texte officiel complet.`};});}
...
export async function POST(request:Request){
  ...
  if(!rawItems.length||!dossiers.length)return NextResponse.json({assignments:[],engine:"myvor-full-text-v4",enriched:0,full_text_chars:0});
  ...
  return NextResponse.json({assignments,engine:"myvor-full-text-v4",enriched,full_text_chars:fullTextChars});
}
