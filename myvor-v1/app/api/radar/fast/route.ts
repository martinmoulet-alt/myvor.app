...
export async function POST(request:Request){
  try{
    ...
    const actors=buildSeeds(dossier,items).map((seed,index)=>{
      ...
      return {id:`${seed.origin}-${index+1}`,name:seed.name,role:seed.role,institution:seed.institution||"",orbit:(index<2?1:index<4?2:3) as 1|2|3,position:"inconnue",position_reason:"Aucune position explicite n’est documentée dans les données actuellement rattachées au dossier.",influence:Math.max(1,Math.min(5,Math.ceil(score.total/20))),influence_score:score.total,score_breakdown:{institutional_power:score.institutional,dossier_relevance:score.relevance,timing:score.timing,accessibility:score.accessibility},why:seed.origin==="source"?`Institution directement reliée à ${sourceCount} source${sourceCount>1?"s":""} officielle${sourceCount>1?"s":""} du dossier.`:signals.length?`Acteur renseigné dans la fiche stratégique et cité dans ${signals.length} évolution${signals.length>1?"s":""}.`:"Acteur renseigné dans la fiche stratégique du dossier ; sa qualification doit être consolidée par les sources.",window:deadline,action:seed.origin==="source"?"Identifier les décideurs ou relais pertinents, documenter leur position puis préparer l’approche avant la prochaine échéance.":"Vérifier sa fonction, documenter sa position et préparer une action adaptée à la prochaine échéance.",certainty:seed.certainty,signals,source_count:sourceCount,evidence:{source_index:source?Math.max(1,items.findIndex(item=>item.id===source.id)+1):0,source_title:source?.title||"Fiche stratégique du dossier",source_url:source?.source_url||"",excerpt:seed.origin==="source"?`Institution identifiée à partir de la source officielle « ${source?.title||"source liée"} ».`:"Acteur issu de la fiche stratégique du dossier.",confidence:seed.certainty==="confirme"?0.95:source?0.7:0.55,verified:seed.origin==="source"&&Boolean(source?.source_url)},contact_verified:false};
    });
