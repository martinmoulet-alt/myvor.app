...
export default function DossiersCorporate({items,watch,add,open,launch}:Props){
  ...
  const filtered=useMemo(()=>items.filter(d=>{
    ...
    return !q||`${d.client} ${d.title} ${d.objective} ${d.context||""}`.toLocaleLowerCase("fr-FR").includes(q);
  }).sort((a,b)=>new Date(b.created_at).getTime()-new Date(a.created_at).getTime()),[items,query,revision]);
  const selected=items.find(d=>d.id===selectedId)||filtered[0]||null;
  const related=useMemo(()=>selected?watch.filter(w=>w.dossier_id===selected.id):[],[selected,watch]);
  ...
  useEffect(()=>{
    if(!selected||!profileEmpty(selected)||attempted.current.has(selected.id))return;
    attempted.current.add(selected.id);
    const timer=setTimeout(()=>void generateStrategy(selected,true),350);
    return()=>clearTimeout(timer);
    // Automatic generation is intentionally keyed to the selected dossier/revision only.
  ...
  async function generateStrategy(dossier:Dossier,automatic=false){
    ...
    setGenerating(true);
    setGenerationMessage(automatic?"Myvor génère automatiquement la fiche stratégique…":"Régénération de la fiche stratégique…");
    try{
      ...
      const payload={
        ...
        strategic_issues:list(p.strategic_issues),
        risks_to_avoid:list(p.risks_to_avoid),
        opportunities:list(p.opportunities),
        client_position:p.client_position||null,
        key_actors:list(p.key_actors),
        watch_topics:list(p.watch_topics),
        watch_subtopics:list(p.watch_subtopics),
        reference_texts:list(p.reference_texts),
        key_deadlines:list(p.key_deadlines),
      ...
      Object.assign(dossier,updated as Dossier);
      setRevision(v=>v+1);
      setGenerationMessage("Fiche stratégique générée et enregistrée automatiquement.");
      window.dispatchEvent(new Event("pageshow"));
    }catch(error:any){
      setGenerationMessage(`Fiche stratégique non générée : ${error?.message||"erreur inconnue"}`);
    }finally{
      setGenerating(false);
    }
  }
  ...
  async function saveDossier(e:FormEvent){
    ...
    try{
      ...
      if(!payload.client||!payload.title||!payload.objective)throw new Error("Client, intitulé et objectif sont obligatoires.");
      const {data,error}=await supabase.from("dossiers").update(payload).eq("id",editing.id).select("*").single();
      if(error)throw error;
      Object.assign(editing,data as Dossier);
      attempted.current.delete(editing.id);
      setEditing(null);
      setRevision(v=>v+1);
      window.dispatchEvent(new Event("pageshow"));
    }catch(error:any){
      setEditMessage(error?.message||"Enregistrement impossible");
    }finally{
      setSaving(false);
    }
  }
  ...
  return <div className="dossiers-workspace">
    ...
    <div className="workspace">
      <section className="panel">
        ...
        <div className="portfolio">{filtered.length?filtered.map(dossier=><article key={dossier.id} className={`dossier-card ${selected?.id===dossier.id?"selected":""}`} onClick={()=>{setSelectedId(dossier.id);setGenerationMessage("");}}>
          ...
          <p>{dossier.objective||"Objectif à préciser"}</p><div className="meta"><span className={`status ${statusClass(dossier.status)}`}>{dossier.status}</span><span style={{fontSize:9}}>{list(dossier.strategic_issues).length?"Fiche stratégique prête":"Fiche à générer"}</span></div>
      ...
      <section className="panel">{selected?<div className="strategy">
        <div className="strategy-hero"><div><small>Fiche stratégique</small><h2>{selected.title}</h2><p>{selected.client} · {selected.status} · {related.length} texte(s) rattaché(s)</p><p><b>Objectif :</b> {selected.objective||"À préciser"}</p></div><div className="strategy-actions"><button className="gold" onClick={()=>void generateStrategy(selected,false)} disabled={generating}><Sparkles size={13}/> {generating?"Génération…":"Régénérer avec Myvor"}</button><button onClick={()=>launch("impact",selected)}><Gauge size={13}/> Score d’urgence</button><button onClick={()=>launch("radar",selected)}><Network size={13}/> Radar & War Zone</button><button onClick={()=>launch("builder",selected)}><FilePenLine size={13}/> Note Builder</button><button onClick={()=>open(selected)}>Ouvrir le dossier complet</button></div></div>
        {generationMessage&&<div className="message">{generationMessage}</div>}
        <div className="strategy-grid">
          <div className="strategy-card"><h3><Building2 size={15}/> 1. Secteurs d’activité</h3><p>{selected.sector||"Non renseigné."}</p></div>
          <div className="strategy-card"><h3><BriefcaseBusiness size={15}/> 2. Activité</h3><p>{selected.activity||"Non renseignée."}</p></div>
          <div className="strategy-card wide"><h3><Target size={15}/> 3. Enjeux stratégiques</h3>{chipList(selected.strategic_issues)}</div>
          <div className="strategy-card"><h3><ShieldAlert size={15}/> 4. Risques</h3>{chipList(selected.risks_to_avoid)}</div>
          <div className="strategy-card"><h3><TrendingUp size={15}/> 5. Opportunités</h3>{chipList(selected.opportunities)}</div>
          <div className="strategy-card wide"><h3><Target size={15}/> 6. Positionnement client</h3><p>{selected.client_position||"Non renseigné."}</p></div>
          <div className="strategy-card"><h3><Users size={15}/> 7. Acteurs clés</h3>{chipList(selected.key_actors)}</div>
          <div className="strategy-card"><h3><Search size={15}/> 8. Thèmes surveillés</h3>{chipList(selected.watch_topics)}</div>
          <div className="strategy-card"><h3><ListTree size={15}/> 9. Sous-thèmes</h3>{chipList(selected.watch_subtopics)}</div>
          <div className="strategy-card"><h3><Quote size={15}/> 10. Expressions prioritaires</h3>{chipList(selected.watch_priority_phrases,"Aucune expression prioritaire renseignée.")}</div>
          <div className="strategy-card"><h3><Ban size={15}/> 11. Expressions à exclure</h3>{chipList(selected.watch_excluded_keywords,"Aucune expression à exclure renseignée.")}</div>
          <div className="strategy-card"><h3><BookOpen size={15}/> 12. Textes de référence</h3>{chipList(selected.reference_texts)}</div>
          <div className="strategy-card"><h3><CalendarDays size={15}/> 13. Échéances</h3>{chipList(selected.key_deadlines)}</div>
