...
export default function DossiersCorporate({items,watch,add,search,searching,messages,open}:{items:Dossier[];watch:Watch[];add:()=>void;search:(d:Dossier)=>void;searching:string|null;messages:Record<string,string>;open:(d:Dossier)=>void}){
  ...
  const alertRank=(dossierId:string)=>{
    const dossierWatch=watch.filter(w=>w.dossier_id===dossierId);
    if(dossierWatch.some(w=>w.urgency==="absolument urgent"))return 2;
    if(dossierWatch.some(w=>w.urgency==="fort"))return 1;
    return 0;
  };
  ...
  const filtered=useMemo(()=>items
    ...
    .sort((a,b)=>alertRank(b.id)-alertRank(a.id)),[items,watch,query,filter]);
