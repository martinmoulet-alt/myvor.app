...
export default function DossiersCorporate({items,watch,add,search,searching,messages,open}:{items:Dossier[];watch:Watch[];add:()=>void;search:(d:Dossier)=>void;searching:string|null;messages:Record<string,string>;open:(d:Dossier)=>void}){
  ...
  const filtered=useMemo(()=>items.filter(dossier=>{
    ...
    const matches=!q||[dossier.client,dossier.title,dossier.objective,dossier.context].join(" ").toLocaleLowerCase("fr").includes(q);
    const rel=watch.filter(item=>item.dossier_id===dossier.id);
  ...
  function beginEdit(dossier:Dossier){
    setMenuId(null);setEditMessage("");setEditing(dossier);
  ...
  async function saveDossier(e:React.FormEvent){
    ...
    Object.assign(editing,data as Dossier);setEditing(null);setRevision(v=>v+1);setSaving(false);
  ...
  async function removeDossier(dossier:Dossier){
    ...
    if(!confirmed)return;
    setDeleting(dossier.id);setDeleteMessage("");setMenuId(null);
    try{
      const {error:actionsError}=await supabase.from("actions").delete().eq("dossier_id",dossier.id);if(actionsError)throw actionsError;
      const {error:dossierError}=await supabase.from("dossiers").delete().eq("id",dossier.id);if(dossierError)throw dossierError;
      window.location.reload();
    }catch(error:any){setDeleteMessage(`Impossible de supprimer le dossier : ${error?.message||"erreur inconnue"}`);setDeleting(null);}
  }

  return <div className={styles.page}>
    <div className={styles.head}>
      <div><h1>Dossiers clients</h1><p>Gérez et suivez l’ensemble de vos dossiers clients.</p></div>
    ...
    <div className={styles.kpis}>
      <button onClick={()=>setFilter("active")} className={styles.kpi}><span className={styles.kpiIcon}><Folder size={22}/></span><span><strong>{activeCount}</strong><small>Dossiers actifs</small><em>Portefeuille client</em></span></button>
      <button onClick={()=>setFilter("all")} className={styles.kpi}><span className={styles.kpiIcon}><FileText size={22}/></span><span><strong>{watch.length}</strong><small>Textes suivis</small><em>{linkedCount} dossier(s) liés</em></span></button>
      <button onClick={()=>setFilter("urgent")} className={styles.kpi}><span className={`${styles.kpiIcon} ${styles.alertIcon}`}><Bell size={22}/></span><span><strong>{urgentCount}</strong><small>Alertes critiques</small><em>À traiter</em></span></button>
      <button onClick={()=>setFilter("all")} className={styles.kpi}><span className={styles.kpiIcon}><BriefcaseBusiness size={22}/></span><span><strong>{items.length}</strong><small>Dossiers au total</small><em>Base active</em></span></button>
    </div>

    <div className={styles.toolbar}>
      <label className={styles.search}><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Rechercher un dossier, un client…"/></label>
      <div className={styles.filters}>
        <button className={filter==="all"?styles.active:""} onClick={()=>setFilter("all")}>Tous les statuts</button>
        <button className={filter==="active"?styles.active:""} onClick={()=>setFilter("active")}>Actifs</button>
        <button className={filter==="urgent"?styles.active:""} onClick={()=>setFilter("urgent")}>Prioritaires</button>
      </div>
    </div>
      ...
      <div className={styles.tableBody}>{filtered.map(dossier=>{
        ...
        return <div className={styles.tableRow} key={dossier.id} onDoubleClick={()=>open(dossier)}>
          ...
          <div className={styles.menuCell}><button className={styles.menuButton} onClick={()=>setMenuId(menuId===dossier.id?null:dossier.id)} aria-label="Actions du dossier"><MoreHorizontal size={19}/></button>{menuId===dossier.id&&<div className={styles.menu}>
            <button onClick={()=>open(dossier)}>Ouvrir le dossier</button>
            <button disabled={!!searching} onClick={()=>{setMenuId(null);search(dossier);}}><Sparkles size={14}/>{searching===dossier.id?"Analyse en cours…":"Chercher les évolutions"}</button>
            <button onClick={()=>beginEdit(dossier)}>Modifier le dossier</button>
            <button className={styles.danger} disabled={deleting===dossier.id} onClick={()=>void removeDossier(dossier)}><Trash2 size={14}/>{deleting===dossier.id?"Suppression…":"Supprimer"}</button>
          </div>}</div>
    ...
    {editing&&<div className={styles.modalBackdrop} onMouseDown={e=>{if(e.target===e.currentTarget&&!saving)setEditing(null);}}><form className={styles.modal} onSubmit={saveDossier}>
      <div className={styles.modalHead}><div><small>Modifier le dossier</small><h2>{editing.title}</h2></div><button type="button" onClick={()=>setEditing(null)}>×</button></div>
      {editMessage&&<div className={styles.message}>{editMessage}</div>}
      <label>Client<input required value={draft.client} onChange={e=>setDraft({...draft,client:e.target.value})}/></label>
      <label>Intitulé du dossier<input required value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/></label>
      <label>Objectif du client<textarea required value={draft.objective} onChange={e=>setDraft({...draft,objective:e.target.value})}/></label>
      <label>Contexte<textarea value={draft.context} onChange={e=>setDraft({...draft,context:e.target.value})}/></label>
      <label>Statut<select value={draft.status} onChange={e=>setDraft({...draft,status:e.target.value})}><option>Actif</option><option>En pause</option><option>Terminé</option><option>Archivé</option></select></label>
      <div className={styles.modalActions}><button type="button" onClick={()=>setEditing(null)} disabled={saving}>Annuler</button><button className={styles.save} disabled={saving}>{saving?"Enregistrement…":"Enregistrer"}</button></div>
    </form></div>}
  </div>;
}