...
type Watch={id:string;title:string;nature:string;source_url:string;dossier_id:string|null;urgency:string;created_at:string;published_at?:string|null;source_name?:string|null;suggested_dossier_id?:string|null;qualification_confidence?:number|null;qualification_reason?:string|null;link_justification?:LinkJustification|null;link_justification_engine?:string|null;link_justified_at?:string|null};
...
export default function DossiersCorporate({items,watch,add,search,searching,messages,open,launch}:{items:Dossier[];watch:Watch[];add:()=>void;search:(d:Dossier)=>void;searching:string|null;messages:Record<string,string>;open:(d:Dossier)=>void;launch:(target:ModuleTarget,dossier:Dossier,watchIds?:string|string[])=>void}){
  ...
  function renderLinkJustification(item:Watch,reason:string){
    const j=item.link_justification;
    const evidence=Array.isArray(j?.evidence)?j!.evidence!.filter(Boolean).slice(0,2):[];
    return <div className="myvor-dossier-link-why"><b>Pourquoi ce rattachement ?</b><p>{j?.summary||reason}</p>{j?.objective_link&&<small><strong>Lien avec le dossier</strong>{j.objective_link}</small>}{evidence.length>0&&<small><strong>Preuves</strong>{evidence.join(" · ")}</small>}{j?.consequence&&<small><strong>Conséquence</strong>{j.consequence}</small>}</div>;
  }
