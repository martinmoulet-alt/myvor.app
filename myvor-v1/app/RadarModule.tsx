...
function bubbleSize(influence:number){
  const safe=Math.max(1,Math.min(5,Math.round(Number(influence)||1)));
  return 114+(safe*14);
}

function bubbleFontSize(name:string,size:number){
  const length=String(name||"").length;
  if(length>72)return Math.max(9.2,size/18);
  if(length>54)return Math.max(9.8,size/17);
  if(length>38)return Math.max(10.5,size/16);
  if(length>24)return Math.max(11.2,size/15);
  return Math.max(12,size/14);
}

function orbitMotion(actor:Actor,actors:Actor[]){
  const same=actors.filter(item=>item.orbit===actor.orbit);
  const rank=Math.max(0,same.findIndex(item=>item.id===actor.id));
  const duration=ORBIT_DURATIONS[actor.orbit];
  const delay=-(duration*(rank/Math.max(1,same.length)))-(actor.orbit*0.7);
  const direction=actor.orbit===2?"reverse":"normal";
  return{radius:ORBIT_RADII[actor.orbit],duration,delay,direction};
}
...
export default function RadarModule({dossiers,watch,onActions}:{dossiers:Dossier[];watch:Watch[];onActions?:(drafts:ActionDraft[])=>Promise<void>|void}){
    ...
    try{
      ...
      if(nextActors.length){
        const saved=await saveProduction({
          ...
          type:"radar",
          title:`Radar d’influence — ${dossier.title}`,
          content:{actors:nextActors,item_ids:related.map(i=>i.id),source_url:source?.source_url||null},
        });
      ...
      if(onActions&&nextActors.length){
        ...
        const drafts=nextActors
          ...
          .map(a=>({
            dossier_id:dossier.id,
            type:"contact",
            title:`Contacter ${a.name}`,
            description:a.action||a.why,
            actor_name:a.name,
            priority:priority(a),
            due_date:null,
          }));
  ...
  return <div className={styles.page}>
    <div className={styles.head}>
      <div>
        <div className={styles.kicker}>Cartographie stratégique</div>
        <h1>Radar d’influence</h1>
        <p>Visualisez les acteurs, leur proximité avec la décision, leur influence et leur position face à l’objectif client.</p>
      </div>
    </div>
    ...
    <div className={styles.setup}>
      <section className={styles.panel}>
        ...
        <div className={styles.field}>
          ...
          <select value={dossierId} onChange={e=>{setDossierId(e.target.value);setActors([]);setSelected(null);setError("");setSaveMessage("");}}>
            <option value="">Sélectionner un dossier</option>
            {dossiers.map(d=><option key={d.id} value={d.id}>{d.client} — {d.title}</option>)}
          </select>
        </div>
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.orbitList}>
          <div className={styles.orbitRow}><b>1re orbite</b><span>Décision directe</span></div>
          <div className={styles.orbitRow}><b>2e orbite</b><span>Influence forte</span></div>
          <div className={styles.orbitRow}><b>3e orbite</b><span>Influence indirecte</span></div>
        </div>
      </section>
    </div>

    <div className={styles.generate}>
      <div><h3>Prêt à cartographier</h3><p>{related.length} texte(s) lié(s) au dossier sélectionné.</p></div>
      <button onClick={generate} disabled={loading||!dossier||!related.length}><Sparkles size={17}/>{loading?"Cartographie en cours…":"Générer le radar d’influence"}</button>
    </div>
    ...
    <section className={styles.radarCard}>
      <div className={styles.legend}>
        <Legend color={COLORS.favorable} label="Favorable"/>
        <Legend color={COLORS.inconnue} label="Neutre ou inconnue"/>
        <Legend color={COLORS.reserve} label="Réserves"/>
        <Legend color={COLORS.opposition} label="Opposition forte"/>
      </div>
      ...
      {actors.length?
        <div className={styles.canvas}>
          ...
          <button
            ...
            style={{cursor:source?.source_url?"pointer":"default"}}
          >
            <span><Orbit size={26}/><b>{dossier?.title||"Dossier"}</b>{source?.source_url&&<small>Lire le texte original</small>}</span>
          </button>

          {actors.map(actor=>{
            const motion=orbitMotion(actor,actors);
            const size=bubbleSize(actor.influence);
            const fontSize=bubbleFontSize(actor.name,size);
            const animationStyle={animationDuration:`${motion.duration}s`,animationDelay:`${motion.delay}s`,animationDirection:motion.direction as "normal"|"reverse"};

            return <div key={actor.id} className={styles.actorOrbit} style={animationStyle}>
              <div className={styles.actorTravel} style={{transform:`translateX(${motion.radius}px)`}}>
                <div className={styles.actorCounter} style={animationStyle}>
                  <button
                    className={styles.actor}
                    onClick={()=>setSelected(actor)}
                    title={`${actor.name} — ${labelCertainty(actor.certainty)}`}
                    style={{
                      width:size,
                      height:size,
                      background:COLORS[actor.position],
                      opacity:actor.certainty==="a_confirmer"?.72:1,
                      fontSize,
                    }}
                  >
      ...
      :
        <div className={styles.empty}><Orbit size={36}/><h3>Le radar est prêt</h3><p>Sélectionnez un dossier puis générez la cartographie des acteurs.</p></div>
      }
    </section>

    {selected&&<div className={styles.modal} onClick={()=>setSelected(null)}>
      <div className={styles.modalBox} onClick={e=>e.stopPropagation()}>
        <div className={styles.modalHead}>
          <div>
            <div className={styles.eyebrow}>Acteur — orbite {selected.orbit} — {labelCertainty(selected.certainty)}</div>
            <h2>{selected.name}</h2>
            <p>{selected.role}</p>
          </div>
          <button className={styles.close} onClick={()=>setSelected(null)}><X size={18}/></button>
        </div>

        <div className={styles.infoGrid}>
          <Info title="Pourquoi il compte" text={selected.why}/>
          <Info title="Éléments d’appui" text={selected.evidence}/>
          ...
          <div className={styles.info}>
            ...
            <p><b>Influence :</b> {selected.influence}/5<br/><b>Proximité décisionnelle :</b> orbite {selected.orbit}<br/><b>Position :</b> {labelPosition(selected.position)}<br/><b>Niveau de certitude :</b> {labelCertainty(selected.certainty)}</p>
          </div>
        </div>
