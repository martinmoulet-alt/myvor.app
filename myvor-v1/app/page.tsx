...
type ImpactResult = { success?:boolean; summary?:string; impact_score?:number; level?:string; risks?:string[]; opportunities?:string[]; recommendations?:string[]; error?:string };
...
function ImpactModule({dossiers,watch}:{dossiers:Dossier[],watch:Watch[]}){
  ...
  function chooseWatch(id:string){
    setWatchId(id);
    const item=watch.find(w=>w.id===id);
    if(item){
      setTitle(item.title);
      const dossier=dossiers.find(d=>d.id===(item.dossier_id||dossierId));
      const dossierContext=dossier?`\n\nDossier client : ${dossier.client} — ${dossier.title}\nObjectif client : ${dossier.objective}\nContexte : ${dossier.context||"Non renseigné"}`:"";
      setContent(`${item.nature} : ${item.title}${dossierContext}`);
      if(item.dossier_id)setDossierId(item.dossier_id);
    }
  }
  ...
  function chooseDossier(id:string){
    ...
    if(dossier && !content){
      setContent(`Dossier client : ${dossier.client} — ${dossier.title}\nObjectif client : ${dossier.objective}\nContexte : ${dossier.context||"Non renseigné"}`);
    }
  }
  ...
  async function generate(e:React.FormEvent){
    e.preventDefault();
    if(!supabase)return;
    if(!title.trim() && !content.trim()){setError("Ajoute au minimum un titre ou un contenu à analyser.");return;}
    setGenerating(true);setError("");setResult(null);
    try{
      const dossier=selectedDossier;
      const enrichedContent=[content.trim(),dossier?`Client : ${dossier.client}\nDossier : ${dossier.title}\nObjectif : ${dossier.objective}\nContexte : ${dossier.context||"Non renseigné"}`:""].filter(Boolean).join("\n\n");
      const {data,error:invokeError}=await supabase.functions.invoke("rapid-task",{body:{title:title.trim()||"Analyse Myvor",content:enrichedContent}});
      if(invokeError)throw invokeError;
      if(!data?.success)throw new Error(data?.error||"La génération de la Note d’impact a échoué.");
      setResult(data as ImpactResult);
    }catch(err:any){
      setError(err?.message||"Impossible de générer la Note d’impact.");
    }finally{
      setGenerating(false);
    }
  }

  const levelClass=result?.level==="faible"?"green":result?.level==="moyen"?"orange":result?.level==="fort"?"red":"wine";

  return <><Header eyebrow="Analyse stratégique" title="Note d’impact" lead="Transformez un texte en risques, opportunités et recommandations opérationnelles."/>
    <div className="grid two">
      <div className="card">
        <h2>Nouvelle analyse</h2>
        <form className="form" onSubmit={generate}>
          <div className="field"><label>Dossier client</label><select value={dossierId} onChange={e=>chooseDossier(e.target.value)}><option value="">Aucun dossier sélectionné</option>{dossiers.map(d=><option key={d.id} value={d.id}>{d.client} — {d.title}</option>)}</select></div>
          <div className="field"><label>Élément de veille</label><select value={watchId} onChange={e=>chooseWatch(e.target.value)}><option value="">Analyse libre / coller un texte</option>{watch.map(w=><option key={w.id} value={w.id}>{w.nature} — {w.title}</option>)}</select></div>
          <div className="field"><label>Titre</label><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Ex. Projet de loi relatif à…"/></div>
          <div className="field"><label>Contenu à analyser</label><textarea style={{minHeight:220}} value={content} onChange={e=>setContent(e.target.value)} placeholder="Collez ici le texte, l’amendement, le décret ou les passages utiles…"/></div>
          {error&&<div className="notice">{error}</div>}
          <button className="btn primary" disabled={generating}><Sparkles size={16}/> {generating?"Analyse en cours…":"Générer la Note d’impact"}</button>
        </form>
      </div>
      <div className="card">
        {!result?<div className="empty"><Sparkles size={34}/><h2>Résultat Myvor</h2><p className="muted">Le résumé, le score, les risques, les opportunités et les recommandations apparaîtront ici.</p></div>:<div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:14}}><div><div className="eyebrow">Score d’impact</div><strong style={{fontSize:38}}>{result.impact_score ?? "—"}<span className="muted" style={{fontSize:18}}>/100</span></strong></div><span className={`badge ${levelClass}`}>{String(result.level||"non classé").replaceAll("_"," ")}</span></div>
          <h3>Résumé</h3><p>{result.summary}</p>
          <ImpactList title="Risques" items={result.risks}/>
          <ImpactList title="Opportunités" items={result.opportunities}/>
          <ImpactList title="Recommandations" items={result.recommendations}/>
        </div>}
      </div>
    </div>
  </>;
...
function ImpactList({title,items}:{title:string,items?:string[]}){return <div style={{marginTop:18}}><h3>{title}</h3>{items?.length?<div className="list">{items.map((item,i)=><div className="row" key={`${title}-${i}`}><span>{item}</span></div>)}</div>:<p className="muted small">Aucun élément.</p>}</div>;}
function ModulePlaceholder({tab,dossiers,watch}:{tab:Tab,dossiers:Dossier[],watch:Watch[]}){const names:any={radar:["Cartographie","Radar d’influence","Positionnez les acteurs par rapport à l’objectif précis du client."],builder:["Production","Note Builder","Préparez une note, un argumentaire ou un e-mail à partir du dossier."]};const n=names[tab]||["Module","Myvor","Module en préparation."];return <><Header eyebrow={n[0]} title={n[1]} lead={n[2]}/><div className="card empty"><Sparkles size={34}/><h2>Module prêt à être connecté au moteur d’analyse</h2><p className="muted">La structure fonctionnelle est en place. Il utilisera les {dossiers.length} dossier(s) et {watch.length} élément(s) de veille de votre espace.</p><span className="badge orange">Étape suivante de la V1</span></div></>;}
