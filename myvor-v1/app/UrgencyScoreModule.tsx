...
import {AlertTriangle,ArrowRight,FileText,Radar,Sparkles,Target} from "lucide-react";
...
export default function UrgencyScoreModule({dossiers,watch,onActions,onOpenRadar,onCreateDeliverable}:{dossiers:Dossier[];watch:Watch[];onActions?:(drafts:ActionDraft[])=>Promise<void>|void;onOpenRadar?:(dossierId:string,watchIds:string[])=>void;onCreateDeliverable?:(dossierId:string,watchIds:string[])=>void}){
  ...
  const defaultIds=useMemo(()=>related.slice(0,MAX_ITEMS).map(item=>item.id),[related]);
  ...
  return <div className={styles.page}>
    <header className={styles.head}><div><div className={styles.kicker}>Décision opérationnelle</div><h1>Score d’urgence</h1><p>Une analyse approfondie unique pour déterminer si une action est nécessaire maintenant, pourquoi elle l’est et quel chemin opérationnel ouvrir ensuite.</p></div></header>
    ...
    <section className={styles.modePanel}><div><b>Analyse approfondie</b><span>Mode unique Myvor : six critères justifiés, preuves, incertitudes, pistes de travail et actions immédiates.</span></div></section>
    ...
    <section className={styles.launch}><div><Sparkles size={21}/><div><b>{selected.length} élément(s) de veille prêts</b><span>Myvor analyse les six critères en profondeur puis recalcule le total côté moteur.</span></div></div><button onClick={generate} disabled={loading||!dossier||!selected.length}>{loading?"Analyse approfondie en cours…":"Calculer le Score d’urgence"}</button></section>
    ...
    {result&&<section className={styles.result}>
      <div className={styles.hero}><div><div className={styles.eyebrow}>Décision Myvor · Analyse approfondie</div><h2>{result.action_needed?"Une action est nécessaire":"Pas d’action immédiate requise"}</h2><p>{result.summary}</p><div className={`${styles.decision} ${tone(result.level)}`}>{result.level.replaceAll("_"," ")} · {result.decision.replaceAll("_"," ")}</div></div><div className={styles.score}><span>Score d’urgence</span><strong>{Math.round(result.score)}</strong><small>/100</small><em>{criteriaTotal===Math.round(result.score)?"6 critères cohérents":"Total recalculé par Myvor"}</em></div></div>
      ...
      {result.workstreams?.length?<section className={styles.section}><h3>Pistes de travail</h3><div className={styles.list}>{result.workstreams.map((item,index)=><div key={index}><span>{index+1}</span><p>{item}</p></div>)}</div></section>:null}
      ...
      {result.uncertainties?.length?<section className={styles.section}><h3>Points à confirmer</h3><div className={styles.list}>{result.uncertainties.map((item,index)=><div key={index}><span>?</span><p>{item}</p></div>)}</div></section>:null}
