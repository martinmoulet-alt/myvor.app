...
export default function WorkspacePage(){
  ...
  async function switchWorkspace(id:string){if(!supabase||busy||!currentUserId)return;setBusy(true);setMessage("");const{error}=await supabase.from("user_profiles").update({active_organization_id:id,updated_at:new Date().toISOString()}).eq("user_id",currentUserId);setBusy(false);if(error){setMessage(error.message);return;}setActiveId(id);setMessage("Workspace actif modifié. Revenez à Myvor pour afficher ses dossiers.");}
  async function createWorkspace(){if(!supabase||busy||!newName.trim())return;setBusy(true);setMessage("");const{data,error}=await supabase.functions.invoke("organization-create",{body:{name:newName.trim()}});setBusy(false);if(error||data?.error){setMessage(String(data?.error||error?.message||"Création impossible."));return;}setNewName("");setMessage("Workspace créé et activé.");await load();}
  async function invite(){if(!supabase||busy||!activeId||!email.trim())return;setBusy(true);setMessage("");const{data,error}=await supabase.functions.invoke("organization-invite",{body:{organization_id:activeId,email:email.trim(),role}});setBusy(false);if(error||data?.error){setMessage(String(data?.error||error?.message||"Invitation impossible."));return;}setEmail("");setMessage(String(data?.message||"Invitation envoyée."));await load();}
