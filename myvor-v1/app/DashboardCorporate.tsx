...
export default function DashboardCorporate({dossiers,watch,actions,actionsLoading,actionsError,go}:{dossiers:Dossier[];watch:Watch[];actions:Action[];actionsLoading:boolean;actionsError:string;go:(tab:Tab)=>void}){
  ...
  const semanticFeed=useSemanticWatchFeed({watch,currentUserId,activeOrganizationId,pinnedDossierId,topics:profile.topics,institutions:profile.institutions});

  const relevantWatch=useMemo(()=>{
    if(semanticFeed.status==="ready"&&semanticFeed.ranks.length){
      const ranked=semanticFeed.ranks.map(result=>watch.find(item=>item.id===result.id)).filter((item):item is Watch=>Boolean(item));
      if(ranked.length){const seen=new Set(ranked.map(item=>item.id));return[...ranked,...urgentWatch.filter(item=>!seen.has(item.id))].slice(0,3);}
    }
  ...
  },[profile.topics,profile.institutions,watch,urgentWatch,semanticFeed.status,semanticFeed.ranks]);
  ...
  return <div className="personal-dashboard">
    ...
    <section className="pd-grid">
      ...
      <article className="pd-panel"><div className="pd-panel-head"><h3><ShieldCheck size={15}/> Votre veille</h3><button className="pd-link" type="button" onClick={()=>go("veille")}>Ouvrir <ArrowRight size={13}/></button></div><div className="pd-body"><div className="pd-config"><div className="pd-config-block"><strong>Sujets suivis</strong><div className="pd-chips">{profile.topics?.length?profile.topics.map(topic=><span className="pd-chip" key={topic}>{topic}</span>):<span className="pd-chip">À personnaliser</span>}</div></div><div className="pd-config-block"><strong>Institutions surveillées</strong><div className="pd-chips">{profile.institutions?.length?profile.institutions.map(item=><span className="pd-chip" key={item}>{item}</span>):<span className="pd-chip">Périmètre général</span>}</div></div><div className="pd-config-note"><b>{semanticFeed.status==="ready"?"Veille sémantique active":semanticFeed.status==="loading"?"Personnalisation en cours":`Mode ${alertLabel}`}</b> · {semanticFeed.status==="ready"?"Myvor croise le sens de vos sujets, institutions et dossiers avec l’urgence et la récence.":profile.alert_level==="essential"?"seules les évolutions décisives remontent.":profile.alert_level==="realtime"?"toute évolution pertinente remonte immédiatement.":"les changements susceptibles d’affecter vos dossiers sont prioritaires."}</div></div></div></article>
    </section>
