...
export default function DashboardCorporate({dossiers,watch,actions,actionsLoading,actionsError,go}:{dossiers:Dossier[];watch:Watch[];actions:Action[];actionsLoading:boolean;actionsError:string;go:(tab:Tab)=>void}){
  ...
  return <div className="corp-dashboard">
    <style jsx global>{`
      ...
      .today-metrics button{position:relative;cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent;transition:transform .12s ease,background .12s ease,border-color .12s ease}
      .today-metrics button:active{transform:scale(.97);background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.22)}
      .today-metrics button:focus-visible{outline:2px solid #f3bd3e;outline-offset:2px}
