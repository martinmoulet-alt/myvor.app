    ...
    const enhance=()=>{
      ...
      if(!result.querySelector('[data-myvor-thematic-radar]')){
        ...
        if(metrics.length>=6){
          ...
          section.innerHTML=`<div class="myvorImpactThematicHead"><div><span>Lecture client</span><b>Impact par thématique</b><small>Vue de synthèse dérivée de la grille Myvor : réglementaire = juridique ; économique = économique/opérationnel ; opérationnel = moyenne économique + capacité d’action ; politique = politique/réputation ; réputation = moyenne politique + probabilité ; stratégique = moyenne capacité d’action + probabilité + urgence.</small></div></div><div class="myvorImpactThematicGrid"><div class="myvorImpactThematicRadar"><svg viewBox="0 0 200 200" role="img" aria-label="Radar d’impact par thématique"><polygon points="100,20 169,60 169,140 100,180 31,140 31,60" class="myvorImpactRadarGrid"/><polygon points="100,46 146,73 146,127 100,154 54,127 54,73" class="myvorImpactRadarGrid"/><line x1="100" y1="100" x2="100" y2="20"/><line x1="100" y1="100" x2="169" y2="60"/><line x1="100" y1="100" x2="169" y2="140"/><line x1="100" y1="100" x2="100" y2="180"/><line x1="100" y1="100" x2="31" y2="140"/><line x1="100" y1="100" x2="31" y2="60"/><polygon points="${points}" class="myvorImpactRadarFill"/></svg></div><div class="myvorImpactThemeList">${themes.map(theme=>`<div><span>${escapeHtml(theme.label)}</span><div><i style="width:${theme.value}%"></i></div><b>${theme.value}</b></div>`).join('')}</div></div>`;
